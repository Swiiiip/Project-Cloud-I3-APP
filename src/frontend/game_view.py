...
class BlurmojiView:
    ...
    def render(self):
        with ui.row().classes('w-full h-[calc(100vh-88px)] min-h-0 gap-3 p-3 items-stretch content-stretch overflow-auto flex-wrap'):
            with ui.column().classes('h-full min-h-0 overflow-hidden').style('flex: 2 1 420px; min-width: 320px;'):
                self._image_section.render()
            with ui.column().classes('h-full min-h-0 overflow-hidden').style('flex: 1 1 300px; min-width: 280px;'):
