...
from src.core.service.supported_emojis import SupportedEmojis
...
from src.persistence.file_database_connection import FileDatabaseConnection


class GameController:
    def __init__(self):
        self.emoji_service = EmojiKitchenService()
        self.db = FileDatabaseConnection()
        self.games: dict[str, Union[DailyChallengeMode, EndlessMode]] = {}

    def get_supported_emojis(self) -> SupportedEmojis:
        return SupportedEmojis(emojis=self.emoji_service.supported_emojis)

    def create_daily_challenge(self, game_id: str) -> DailyChallengeStartResult:
        game_state = DailyChallengeState(game_id)
        game_mode = DailyChallengeMode(game_state, self.emoji_service)
    ...
    def create_endless_game(self, game_id: str) -> EndlessModeStartResult:
        ...
        game_mode = EndlessMode(game_state, self.emoji_service)
        self.games[game_id] = game_mode
        return game_mode.start_game()
